...
| 3 | `game/world.js` | 434 | The world as tables and fixed objects: `Body`, `Node`, `LandingSite`, `Depot`, `Connection`, `TransferTable`, goods, ship classes; the starport table and zones of influence a new game starts with; the game rules as constants | game/transfertables |
...
| 5 | `game/state.js` | 350 | `S` — the game as objects: `Game`, `Player`, `Ship`, `Market`, `Starport`, `Hub`, `Industry`, `Store`, `Demand`, `Order`, and `Plan`, `Step`, `Autopilot`; beside the game, the `Leaderboard` | world |
| 6 | `game/save.js` | 129 | Reading a save back: old ids and names mapped, checked, rebuilt as objects | game/state, world |
...
| 13 | `game/commands.js` | 397 | **All commands.** Changes `S`, reports `changed()`; keeps the save and the leaderboard | actions, basics, economy, events, geometry, graph, physics, planner, save, game/state, world |
...
| 21 | `ui/widgets.js` | 61 | Button, icon, chip, panel heading, the pilot's years; `openView`, `openRoute` | basics, events, game/state, ui/state, world |
...
| 24 | `ui/panels.js` | 434 | Trading post, cargo, refuel, shipyard, route; ship and pilot, youth clinic, leaderboard | basics, commands, economy, events, planner, game/state, transfermap, ui/state, widgets, world |
| 25 | `ui/display.js` | 83 | Header, toast, autopilot bar, `render()` | basics, commands, draw, events, panels, pickcard, planner, game/state, ui/state, widgets, world |
| 26 | `ui/menu.js` | 79 | Menu, fullscreen, legend, version line | basics, commands, draw, events, game/state, ui/state, widgets |
...
What the game knows, what the screen shows and what outlives a game live in
separate places, and only the first one is `S`:
...
| `game/commands.js` | `Leaderboard` | Whoever died, the ten best balances. Outlives the game: kept in a cookie of its own, apart from the save, and left as it is by a new game. | the commands, when the player dies |
