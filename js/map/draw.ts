...
import { S, Pick, cargoOrders, here, homePlanet } from '../game/state.js';
...
// path of a manoeuvre: position at fraction t
  ...
  let ship:{pt:ReturnType<typeof P>; dir:number[]; burn?:string|null; soon?:string|null; glow?:boolean; up?:boolean}|null=null;
...
// moons as small spheres, behind or in front
  ...
  front.forEach(f=>f());
  // orbit markers
  const nodeMark=(q:Vec3,node:string,label:string,align:CanvasTextAlign)=>{ const c=P(q), x=c.x, y=c.y, pk:Pick={type:'node',node}, sel=isPick(pk), here_=mine&&S.domain.node===node;
...
// grid: parallels and meridians (front side only)
  ...
  // Orbit markers
  const marker=(p:{x:number;y:number},node:string,label:string,al:CanvasTextAlign)=>{ const pk:Pick={type:'node',node}, sel=isPick(pk);
...
// landing sites (faded on the far side, still tappable)
  (SITES[b]||[]).forEach(st=>{
    ...
    const pk:Pick={type:'node',node:b+'.surf',site:st.id}, sel=isPick(pk), kon=POSTS.find(kk=>kk.node===b+'.surf' && (!kk.site||kk.site===st.id));
