...
function drawSol(){
  ...
  if(T && T.along.transferWindow){
    ...
    const pt=(s:number):[number,number]=>{ const q=conic.at(s), th=th0+q.th; return [c+q.r*Math.cos(th), c-q.r*Math.sin(th)]; };
    ctx.strokeStyle=v('--accent'); ctx.setLineDash([3,4]); ctx.lineWidth=1.5; ctx.beginPath();
    for(let i=0;i<=96;i++){const [x,y]=conic.geo(i/96); const th=th0+y; const X=c+x*Math.cos(th), Y=c-x*Math.sin(th); i?ctx.lineTo(X,Y):ctx.moveTo(X,Y);} ctx.stroke(); ctx.setLineDash([]);
...
// burning at departure (prograde) and on arrival (braking), with the rocket turning beforehand
  ...
  } else { const k=(S.player.ship.near?.planet??null); if(k){
...
// three.js draws the planet, its moons and Saturn's ring; the maths here is in pixels (scale 1).
  ...
  const mine=(S.player.ship.near?.planet??null)===p, st=sysState(p);
...
// moons as small spheres, behind or in front
  ...
  front.forEach(f=>f());
  // orbit markers
  const nodeMark=(q:Vec3,node:NodeId,label:string,align:CanvasTextAlign)=>{ const c=P(q), x=c.x, y=c.y, pk:Pick={type:'node',node}, sel=isPick(pk), here_=mine&&S.player.ship.near?.node===node;
...
// three.js takes care of the sphere; the same camera, R pixels per body radius
  ...
  const gb = bctx; // hidden parts belong behind the sphere
  glPut(b,null,1);
  const place=S.player.ship.near, k=place?.body, l=place?.level, mine=k===b;
  const font=(w:number, sz:number)=>g.font=`${w} ${sz}px 'Saira Semi Condensed','Arial Narrow',sans-serif`;
...
export function idleLoop(ts:number){
  ...
  if(reduce || !S || S.player.ship.underWay || document.hidden || ts-idleT<40){ if(ts-idleT>=40) idleT=ts; return; }
