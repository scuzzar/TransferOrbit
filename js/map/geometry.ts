...
export type Orbit = { [k:string]: any };
export interface Move { from:Target; to:Target; d0:number; d1:number; orb:Orbit|null; aero?:boolean; [k:string]:any; }
...
export const vadd=(a:Vec3,b:Vec3):Vec3=>[a[0]+b[0],a[1]+b[1],a[2]+b[2]], vmul=(a:Vec3,s:number):Vec3=>[a[0]*s,a[1]*s,a[2]*s];

const vdot=(a:Vec3,b:Vec3)=>a[0]*b[0]+a[1]*b[1]+a[2]*b[2], vnorm=(a:Vec3):Vec3=>vmul(a,1/(Math.hypot(...a)||1));
...
export function bodyView(b:string):View{
  ...
  for(const el of [CAM_EL,-CAM_EL]) for(let L=-180; L<180; L+=5){
    ...
    let s=0; st.forEach((x:Site)=>{ const f=x.lat*D2R, d=(x.lon-L)*D2R;
      const z=Math.sin(f)*Math.sin(el)+Math.cos(f)*Math.cos(d)*Math.cos(el); s+=(z>0.12?10:0)+z+(x.port?3*(z>0.12?1:0):0); });
...
export function makeCam(cx:number,cy:number,R:number,el:number):Cam{
  const ce=Math.cos(el), se=Math.sin(el);
  const view=(p:Vec3):Vec3=>[p[0], p[1]*ce-p[2]*se, p[1]*se+p[2]*ce];
// hidden: behind the body
  const proj=(p:Vec3)=>{ const q=view(p); return {x:cx+R*q[0], y:cy-R*q[1], z:q[2], hidden:q[2]<0 && q[0]*q[0]+q[1]*q[1]<1}; };
  return {view, proj, cx, cy, R};
}

// Orbit: inclination i, ascending node Om (longitude), position from the argument u
export function orbitPos(b:string,o:Orbit,u:number,r:number):Vec3{ const N=bvec(b,0,o.Om), Mv=bvec(b,o.i,o.Om+90); return vmul(vadd(vmul(N,Math.cos(u)),vmul(Mv,Math.sin(u))),r); }

// An orbit passing eastwards over a landing site (launch due east)
function siteOrbit(st:{lat:number;lon:number}){ const i=Math.abs(st.lat); return st.lat>=0 ? {i, Om:st.lon-90, uSite:90*D2R} : {i, Om:st.lon+90, uSite:270*D2R}; }
...
export function bodyPath(mv:Move):BodyPath|null{
...
// launch: straight up, then east into the circular orbit
  if(fl==='surf' && tl==='orbit' && fb===tb){
    const st:{lat:number;lon:number} = siteOf(b,mv.from.site??null)||{lat:0,lon:bodyLon0(b)}, o=siteOrbit(st), du=75*D2R;
...
// landing: coast in the old orbit to the braking point (the plane rotates towards the landing track), brake, descend, touch down vertically
  if(fl==='orbit' && tl==='surf' && fb===tb){
    const st:{lat:number;lon:number} = siteOf(b,mv.to.site??null)||{lat:0,lon:bodyLon0(b)}, o=siteOrbit(st), c:Orbit = mv.orb||shipOrb(b), arc=110*D2R;
    ...
    return {b, finalOrb:null, at:t=>{
      if(t<0.38){ const s=t/0.38; const oo:Orbit={body:b, i:c.i+(o.i-c.i)*s, Om:c.Om+dOm*s}; return {p:orbitPos(b,oo,c.u+du*s,R_ORB), burn:s>0.92?'retro':null}; }
      const s=(t-0.38)/0.62;
...
// hop: a ballistic arc along the great circle
  if(fl==='surf' && tl==='surf' && fb===tb){
    const A = siteOf(b,mv.from.site??null), Bs = siteOf(b,mv.to.site??null); if(!A||!Bs) return null;
...
// raise the orbit (Hohmann, half an ellipse at Kepler pace)
  if(fl==='orbit' && tl==='capt' && fb===tb){
    const c:Orbit = mv.orb||shipOrb(b), rp=R_ORB, ra=R_HIGH, a=(rp+ra)/2, e=(ra-rp)/(ra+rp);
