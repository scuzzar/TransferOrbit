...
export function dots(n){ return `<span class="dots">${[0,1,2].map(i=>`<i class="${i<n?'on':''}"></i>`).join('')}</span>`; }

export function btn(label,cls,disabled,fn){ const b=document.createElement('button'); b.textContent=label; if(cls) b.className=cls; b.disabled=disabled; b.onclick=fn; return b; }
...
const icon = (k,sz=18) => `<svg width="${sz}" height="${sz}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${IC[k]}</svg>`;

export function ibtn(ic,label,cls,disabled,fn){ const b=btn('',cls,disabled,fn); b.innerHTML=`${icon(ic)}<span>${label}</span>`; return b; }

export function openRoute(target,back){ S.ui.route={target, mode:'eco'}; openView('route',back); }
