...
import { Order, RouteMode, S, targetName } from '../game/state.js';
...
function panelPost(p:HTMLElement){
  const k=(S.ship.place?.post??null); if(!k){ openView('main'); return; }
  const post=S.market.post(k.id), locked=!S.canAct, free=S.ship.def.slots-S.ship.slotsUsed;
  ...
  const bigHere=offers.filter(o=>o.containers>S.ship.def.slots);
  if(bigHere.length){ const hb=document.createElement('p'); hb.className='hint';
    hb.textContent=`${bigHere.length} bulk ${bigHere.length>1?'orders do':'order does'} not fit in your ${S.ship.def.name} (${S.ship.def.slots} slots). Larger ships are sold at the shipyards of the hubs.`; p.appendChild(hb); }
  ...
  glist.forEach(dest=>{
    ...
    const after = gSel.length ? S.ship.dvWith(S.ship.fuel,S.ship.cargoMass+selM) : S.ship.dvWith(S.ship.fuel,S.ship.cargoMass+selM+(isFinite(lightest)?lightest:0));
    const short = after < dest.dv;
    const afterFull = gSel.length ? S.ship.dvWith(S.ship.def.cap,S.ship.cargoMass+selM) : S.ship.dvWith(S.ship.def.cap,S.ship.cargoMass+selM+(isFinite(lightest)?lightest:0));
    const grp=document.createElement('section'); grp.className='ogroup'+(gSel.length?' on':'');
    const nDel=S.ship.hold.filter(o=>o.to===tk.id).length;
    ...
    os.forEach(o=>{
      const Gd=GOODS[o.good], on=UI.sel.has(o.id), fits=on || S.ship.slotsUsed+selN+o.containers<=S.ship.def.slots;
      const el=document.createElement('label'); el.className='orow'+(on?' on':'')+(fits?'':' dis');
      const tooBig=o.containers>S.ship.def.slots, need=tooBig?shipFor(o.containers):null;
  ...
  const before=S.ship.dvAvail, after=S.ship.dvWith(S.ship.fuel,S.ship.cargoMass+selM);
...
function panelCargo(p:HTMLElement){
  const locked=!S.canAct, co=S.ship.hold, hereK=(S.ship.place?.post??null);
  p.appendChild(phead(`Cargo hold of the ${S.ship.def.name}`, S.ship.place?`Currently in ${esc(S.ship.place.name)}.`:'Under way.', ''));
  ...
  while(cells.length<S.ship.def.slots) cells.push('<div class="free">free</div>');
  slots.innerHTML=cells.join(''); p.appendChild(slots);
  const tot=S.ship.def.dry+S.ship.cargoMass+S.ship.fuel, pct=(x:number)=>(x/tot*100).toFixed(1)+'%';
  ...
  m.innerHTML=`<div class="row"><span class="muted">Mass</span><span>${tons(tot)} total</span></div>
    <div class="massbar"><i style="width:${pct(S.ship.def.dry)};background:var(--muted)"></i><i style="width:${pct(S.ship.cargoMass)};background:#2a6bd1"></i><i style="width:${pct(S.ship.fuel)};background:var(--accent)"></i></div>
    <div class="legendrow"><span><i style="background:var(--muted)"></i>Ship ${tons(S.ship.def.dry)}</span><span><i style="background:#2a6bd1"></i>Cargo ${tons(S.ship.cargoMass)}</span><span><i style="background:var(--accent)"></i>Fuel ${tons(S.ship.fuel)}</span></div>
    <p>Δv with this load: <b>${km(S.ship.dvAvail)} km/s</b></p>`;
  const r=refuelInfo();
  if(r && r.need>0.05){ const a=document.createElement('button'); a.className='linkbtn'; a.textContent=`Filling up raises it to ${km(S.ship.dvWith(S.ship.def.cap,S.ship.cargoMass))} km/s`; a.onclick=()=>openView('refuel'); m.appendChild(a); }
  p.appendChild(m);
  ...
  co.forEach(o=>{
    ...
    if(!(hereK && hereK.id===o.to) && S.ship.place){ const rl=routeLink(to,'cargo'); rl.style.marginRight='auto'; row.appendChild(rl); }
...
function panelRefuel(p:HTMLElement){
  const r=refuelInfo(), place=S.ship.place; if(!r||!place){ openView('main'); return; }
  const locked=!S.canAct;
  p.appendChild(phead(`Refuel`, `${esc(place.name)}. ${r.source}, ${r.price} Cr per t, takes ${fmtDays(r.days)}.`, ''));
  ...
  box.innerHTML=`<div class="row"><label for="tankamt"><b>Amount</b></label><b class="big">${tons(amt)}</b></div>
    ...
    <div class="row muted"><span>Tank ${tons(S.ship.fuel)}</span><span>full ${tons(S.ship.def.cap)}</span></div>`;
  ...
  const forRoute=Math.max(0,Math.min(r.max, S.ship.fuelFor(needDv,S.ship.cargoMass)*1.03-S.ship.fuel));
  ...
  const dvA=S.ship.dvWith(S.ship.fuel+amt,S.ship.cargoMass);
  cmp.innerHTML=`<div><span class="muted">Δv with cargo</span><span>${km(S.ship.dvAvail)} km/s</span><b class="acc">${km(dvA)} km/s</b></div>
  ...
  const reg=REGION[place.body], inReg=(key:string)=>{ const [n]=key.split('@'); return isNode(n) && REGION[splitNode(n)[0]]===reg; };
  ...
  const me=(S.ship.place?.key??null), isMe=(key:string)=>key===me || key===place.node && !(me && FUEL_PRICE[me]);
...
function panelShipyard(p:HTMLElement){
  const k=(S.ship.place?.post??null); if(!k||!k.hub){ openView('main'); return; }
  const locked=!S.canAct, cur=S.ship.def;
  ...
  SHIP_IDS.forEach(id=>{ const sh=SHIPS[id];
    const mine=id===S.ship.type, net=sh.price-0.7*cur.price, fits=S.ship.slotsUsed<=sh.slots, diff=sh.slots-cur.slots;
...
export function renderPlace(){
  ...
  const k=(S.ship.place?.post??null), r=refuelInfo(), del=deliverables(), locked=!S.canAct;
  const c=document.createElement('div'); c.className='place';
  const tr=S.ship.transit, where=S.ship.place?S.ship.place.name:tr?`Under way to ${B[tr.to].name}`:'Under way';
  ...
  if(k){ const n=S.market.post(k.id).offers.length; g.appendChild(ibtn('orders',`Orders (${n})`,del.length?'':'go',S.ship.busy,()=>openView('post'))); }
  if(r) g.appendChild(ibtn('fuel','Refuel','',S.ship.busy,()=>openView('refuel')));
  if(k&&k.hub) g.appendChild(ibtn('yard','Shipyard','',S.ship.busy,()=>openView('shipyard')));
...
function panelRoute(p:HTMLElement){
  ...
  const tr=S.ship.transit;
  const here=S.ship.place;
  if(!here){
    p.appendChild(phead(`Route: ${targetName(R.target)}`, tr?`Under way to ${B[tr.to].name}, arriving ${dateStr(tr.arr)}.`:'Under way.', ''));
    ...
    if(S.ship.autopilot) p.appendChild(autoBtn());
    return;
  }
  if(S.ship.isAt(R.target)){
  ...
  p.appendChild(phead(`Route: ${targetName(R.target)}`, `From ${esc(here.name)}.`, ''));
  if(UI.rmsg && UI.msg){ const m=document.createElement('div'); m.className='msg'; m.style.margin='0 0 12px'; m.textContent=UI.msg; p.appendChild(m); }
  ...
  ([['eco','Economical'],['now','Leave now']] as const).forEach(([m,t])=>{
    const pl=plans[m], ok=pl && pl.dv<=S.ship.dvAvail+0.5;
  ...
  p.appendChild(tl);
  const have=S.ship.dvAvail, ok=plan.dv<=have+0.5; R.strand=false;
  const sum=document.createElement('div'); sum.className='pfoot';
  const deadl=S.ship.hold.filter(o=>{ const k=POST_BY_ID[o.to]; return k.node===R.target.node && (!k.site||k.site===R.target.site); });
  ...
  sum.innerHTML=`<div class="row"><span class="muted">Needs ${km(plan.dv)} of ${km(have)} km/s</span><b class="${ok?'okc':'badc'}">${ok?km(have-plan.dv)+' km/s left':km(plan.dv-have)+' km/s short'}</b></div>
    ...
    <p class="kinfo">${deadl.length?(late.length?`${late.length} ${late.length>1?'orders arrive':'order arrives'} after the deadline.`:`${deadl.length>1?'All '+deadl.length+' orders':'The order'} for this destination ${deadl.length>1?'arrive':'arrives'} before the deadline.`):''}${plan.fee?` Launch fee ${fmtCr(plan.fee)}.`:''} ${ok?'The autopilot stops wherever cargo can be delivered on the way.':S.ship.dvWith(S.ship.def.cap,S.ship.cargoMass)<plan.dv?`Loaded too heavily: even with a full tank you would only have ${km(S.ship.dvWith(S.ship.def.cap,S.ship.cargoMass))} km/s. Return an order or pick another destination.`:'Refuel first, or pick a different route.'}</p>`;
  if(ok){
    const cm=S.ship.cargoMass, m0=S.ship.def.dry+cm+S.ship.fuel, fAfter=Math.max(0, m0/Math.exp(plan.dv/(S.ship.def.isp*G0))-S.ship.def.dry-cm);
    const cmAfter=cm-deadl.reduce((s,o)=>s+o.containers*GOODS[o.good].m,0), rest=S.ship.dvWith(fAfter,cmAfter);
  ...
  if(!ok && !S.ship.autopilot){
    ...
    if(spot && nf.dv>0 && nf.dv<=have+0.5 && !(spot.node===R.target.node && (spot.site||null)===(R.target.site||null))){
      const fb=btn(`To the nearest depot first: ${targetName(spot)}, ${km(nf.dv)} km/s`,'wide',locked,()=>openRoute(spot,UI.back));
      fb.style.fontSize='.92rem'; sum.appendChild(fb);
    }
  }
  // refuelling straight from the route planner, if there is a depot here
  const rf=refuelInfo();
  if(rf && rf.need>0.5 && !S.ship.autopilot){
    const box=document.createElement('div'); box.className='rtank';
    const dvFull=S.ship.dvWith(S.ship.fuel+rf.max,S.ship.cargoMass), full=rf.max>=rf.need-0.05;
    const forRoute=Math.max(0,Math.min(rf.max, S.ship.fuelFor(plan.dv,S.ship.cargoMass)*1.03-S.ship.fuel));
  ...
  g.appendChild(btn('Next step only','',locked||!!S.ship.autopilot||!plan.steps.length,()=>{ const st=plan.steps[0]; if(!st) return; UI.rmsg=true; if(!execStep(st)){ UI.msg=`"${st.label}" is not possible right now. ${stepBlocker(st)}`; } changed(); }));
  if(S.ship.autopilot) g.appendChild(btn('Stop the autopilot','',false,()=>stopAutopilot('Autopilot stopped.')));
