...
export function speedHint(){ const h=$('speedhint') as HTMLElement|null; if(!h) return;
...
function renderAutobar(){
  const w=$('autobar') as HTMLElement;
...
export function wireDisplay(){
  ($('stage') as HTMLElement).addEventListener('pointerdown',()=>{ if(ANIM.active && !ANIM.fast){ ANIM.fast=true; tick(); } });
}