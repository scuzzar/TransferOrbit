...
import { B, BodyId, Connection, GOODS, GoodId, RATE_MASS, LAUNCH_FEE, M, Node, NodeId, PlanetId, SHIP_MASS_SHARE, PLANETS, SITES, TransferTable, TIME_DAYS, V_EXHAUST, VALUE_RATE, hasAtm, isMoon, moonsOf, nodeOf, rotPenalty, siteOf, transferTable } from './world.js';
...
// What an order pays for the delta-v it needs: the propellant and a share of the goods' value that
// grows with the delta-v, both raised by the route's flight time (its days, waits left out), and a
// share of the launch fee where the route starts on a launcher.
export function rewardFor(r: {dv:number; days:number; launch:boolean}, good: GoodId, n: number): number{
  const G=GOODS[good], mass=SHIP_MASS_SHARE+n*G.mass, x=r.dv/V_EXHAUST;
  let R = (RATE_MASS*mass*(Math.exp(x)-1) + VALUE_RATE*n*G.value*x) * (1+r.days/TIME_DAYS);
