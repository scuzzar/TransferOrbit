...
import { BULK, GOODS, HUBS, HUB_CAP, POST_BY_ID, POSTS, MAX_OPEN, MAX_ROUTE_DV, REGION, START_DAY, bodyOf } from './world.js';
...
export function freshDeadline(o,day){ const r=route(POST_BY_ID[o.from],POST_BY_ID[o.to]); return day+legWait(r,day)+1.5*o.days+30; }
...
function bulkTick(day){
  ...
  POSTS.forEach(k=>k.makes.forEach(g=>{
    ...
    const n=N[g];
    const cand=POSTS.filter(c=>c.id!==k.id && c.needs.includes(g) && eco.demand[c.id][g]>0 && bodyOf(c)!==bodyOf(k) && route(k,c).dv<=MAX_ROUTE_DV);
