...
(async()=>{
  ...
  for(const [name,vp] of (process.env.ONLY==='mob'?[['mob',{width:390,height:844}]]:process.env.ONLY==='desk'?[['desk',{width:1440,height:860}]]:[['desk',{width:1440,height:860}],['mob',{width:390,height:844}]])){
    ...
    const click=async(sel,label)=>{ const el=await f.$(sel); if(!el) return false; if(!(await el.isVisible())||await el.isDisabled()) return false;
      ...
      if(how==='rescue') SPEND.where.push(await f.evaluate(()=>{ const r=TO.rescueInfo(); return TO.locKey()+(r&&r.local?'[credit]':'[tanker]')+' cr='+Math.round(TO.S.credits)+' fuel='+TO.S.fuel.toFixed(1)+' cargo='+TO.cargoMass(); }));
      const before=await f.evaluate(()=>TO.S.credits);
      ...
      const d=(await f.evaluate(()=>TO.S.credits))-before;
    ...
    await f.evaluate(()=>{ TO.S.credits=12345; TO.changed(); });
    ...
    // --- 2. save / load
    await f.evaluate(()=>{ TO.S.credits=777; TO.changed(); }); await click('#menubtn'); await click('[data-menu="save"]','Save');
    await f.evaluate(()=>{ TO.S.credits=1; TO.changed(); }); await click('#menubtn'); await click('[data-menu="load"]','Load');
    ...
    for(let i=0;i<STEPS;i++){
      ...
      const tgt=await f.evaluate(()=>{ const cand=TO.POSTS.filter(k=>TO.S.eco.orders.some(o=>o.state==='open'&&o.from===k.id)&&!(TO.postAt()&&TO.postAt().id===k.id));
