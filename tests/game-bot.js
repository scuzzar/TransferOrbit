...
(()=>{
  ...
  function travel(target){
    for(let i=0;i<40;i++){
      ...
      if(TO.S.busy){ log('still busy after a step?!'); }
  ...
  function tryBuy(){
    ...
    for(const id of order.slice().reverse()){
      ...
      if(TO.S.credits>=net+buffer && TO.slotsUsed()<=sh.slots){ TO.buyShip(id); EV.ships.push({ship:id,day:Math.round(TO.S.day-TO.START_DAY),credits:Math.round(TO.S.credits)}); log('BOUGHT '+sh.name+' for '+TO.fmtCr(net)); refuel(); return; }
    }
  }
  window.turn=function(){
    if(TO.S.over){ log('BANKRUPT'); return 'over'; }
    // deliver
    ...
    refuel(); tryBuy();
    if(TO.S.ship==='carrack') return 'done';
  ...
  // nothing possible: head for the nearest depot, or wait
    if(!TO.refuelInfo()){ const nf=TO.nearestFuel({node:TO.S.node,site:TO.S.site,day:TO.S.day}); if(nf.spot && nf.dv<=TO.dvAvail()){ log('heading for the depot '+TO.targetName(nf.spot)); travel(nf.spot); return 'fuel'; } }
...
(async()=>{
  ...
  for(let i=0;i<4000;i++){
    res=await p.evaluate(()=>{ let r; try{ r=turn(); }catch(e){ r='ERR '+e.message+' '+e.stack.split('\n')[1]; } TO.changed(); return {r, day:Math.round(TO.S.day-TO.START_DAY), cr:Math.round(TO.S.credits), ship:TO.S.ship}; });
