...
(async()=>{ const b=await chromium.launch(); const errs=[];
  ...
  await p.evaluate(()=>{ try{localStorage.clear();}catch(e){} TO.newGame(); TO.S.player.ship.fuel=80; TO.changed(); });
  ...
  const move=w=>p.evaluate(w=>{ const a=eval(w); if(!a) return false;
    TO.SCENE.move=TO.planMove(a); TO.S.player.ship.busy=true; return true; },w);
  const shots=[];
  const snap=async(setup,ts,tag)=>{
    ...
    await p.evaluate(()=>{ TO.SCENE.move=null; TO.S.player.ship.busy=false; TO.SCENE.anim=null; TO.changed(); });
  };
  await snap(()=>move("TO.localActions().find(a=>a.site==='kourou')"),[0.1,0.4,0.7,0.9],'land');
  await p.evaluate(()=>{ TO.S.player.ship.dock(new TO.Place('earth.surf','kourou')); TO.S.player.ship.fuel=80; TO.changed(); });
  await snap(()=>p.evaluate(()=>TO.draw()),[0],'pad');
  await snap(()=>move("TO.localActions().find(a=>a.to==='earth.orbit')"),[0.1,0.5,0.9],'up');
  await p.evaluate(()=>{ TO.S.player.ship.dock(new TO.Place('earth.orbit')); TO.S.player.ship.fuel=80; TO.changed(); TO.setView({level:'sys',planet:'earth'}); });
