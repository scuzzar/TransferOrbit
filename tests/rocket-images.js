...
(async()=>{ const b=await chromium.launch(); const errs=[];
  ...
  await p.evaluate(()=>{ try{localStorage.clear();}catch(e){} TO.newGame(); TO.S.fuel=80; TO.changed(); });
  ...
  const move=w=>p.evaluate(w=>{ const a=eval(w); if(!a) return false;
    TO.S.move=TO.planMove(a); TO.S.busy=true; return true; },w);
  const shots=[];
  const snap=async(setup,ts,tag)=>{
    await setup();
    for(const t of ts){
      await p.evaluate(t=>{ const m=TO.S.move; if(m){TO.S.day=m.d0+(m.d1-m.d0)*t;} TO.LAST_ROCKET.layer=null; TO.draw(); },t);
    ...
    await p.evaluate(()=>{ TO.S.move=null; TO.S.busy=false; TO.S.anim=null; TO.changed(); });
  };
  await snap(()=>move("TO.localActions().find(a=>a.site==='kourou')"),[0.1,0.4,0.7,0.9],'land');
  await p.evaluate(()=>{ TO.S.node='earth.surf'; TO.S.site='kourou'; TO.S.fuel=80; TO.changed(); });
  await snap(()=>p.evaluate(()=>TO.draw()),[0],'pad');
  await snap(()=>move("TO.localActions().find(a=>a.to==='earth.orbit')"),[0.1,0.5,0.9],'up');
  await p.evaluate(()=>{ TO.S.node='earth.orbit'; TO.S.site=null; TO.S.fuel=80; TO.changed(); TO.setView({level:'sys',planet:'earth'}); });
