...
(async()=>{ const b=await chromium.launch(); const errs=[];
  ...
  await p.evaluate(()=>{ TO.S.player.ship.dock(TO.nodeOf('earth.surf','kourou')); TO.S.player.ship.fuel=80; TO.changed(); });
  await snap(()=>p.evaluate(()=>TO.draw()),[0],'pad');
  await snap(()=>move("TO.localActions().find(a=>a.to==='earth.orbit')"),[0.1,0.5,0.9],'up');
  await p.evaluate(()=>{ TO.S.player.ship.dock(TO.nodeOf('earth.orbit')); TO.S.player.ship.fuel=80; TO.changed(); TO.setView({level:'sys',planet:'earth'}); });
