...
(async()=>{
  ...
  // Trotzdem starten
  await f.evaluate(()=>{ openRoute({node:'moon.surf',site:'tranquillitatis'}); });
  ...
  await f.evaluate(()=>{ openView('main'); S.node='mars.surf'; S.site='pavonis'; S.credits=0; S.fuel=45; strandCache={key:null}; render(); });
  console.log('Gestrandet bei 0 Cr, 45 t in Pavonis:', await f.evaluate(()=>stranded()));
  await f.evaluate(()=>{ S.fuel=1; strandCache={key:null}; render(); });
  console.log('Gestrandet bei 0 Cr, 1 t in Pavonis:', await f.evaluate(()=>stranded()), '| Box:', (await f.textContent('#rescue')).slice(0,60));
